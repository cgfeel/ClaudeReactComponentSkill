...
- `.claude/setting.local.json` - 本地权限配置 (allow/deny 规则)
- `.claude/setting.json` - 项目级共享配置
- `CLAUDE.md`（全局位于 `~/.claude/`） - 用户级行为指令
- `claude.md`（本文件） - 项目级 Claude Code 指令

## 注意事项

- `.claude/settings.local.json` 应保持与全局 `CLAUDE.md` 红线操作一致的限制
- 敏感文件（.evn、密钥、证书）不要提交到 `Git` 仓库
